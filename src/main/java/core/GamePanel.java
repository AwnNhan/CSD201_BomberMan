...
public class GamePanel extends JPanel implements Runnable {
    ...
    public String direction = "right"; // Thêm dòng này để theo dõi hướng mặc định
                ...
                for (Enemy e : enemyList) {
                    if (assetManager.getSprite("ENEMY") != null) {
                        if ("up".equals(e.direction)) {
                            g2.drawImage(assetManager.getSprite("ENEMY_UP"), (int) e.getX(), (int) e.getY(), tileSize, tileSize, null);
                        } 
                        else if ("down".equals(e.direction)) {
                            g2.drawImage(assetManager.getSprite("ENEMY_DOWN"), (int) e.getX(), (int) e.getY(), tileSize, tileSize, null);
                        }
                        // Lấy biến direction từ con quái để quyết định chiều vẽ
                        else if ("left".equals(e.direction)) {
                            // LẬT ẢNH SANG TRÁI: Dùng chiều rộng âm (-tileSize) và cộng thêm tileSize vào X
                            g2.drawImage(assetManager.getSprite("ENEMY"), (int) e.getX() + tileSize, (int) e.getY(), -tileSize, tileSize, null);
                        } else {
                            // BÌNH THƯỜNG (SANG PHẢI)
                ...
                if (assetManager.getSprite("PLAYER") != null) {
                    ...
                    if (!invincible || System.currentTimeMillis() / 100 % 2 == 0) {
                        if (direction.equals("up")) {
                            ...
                            g2.drawImage(assetManager.getSprite("PLAYER_UP"), playerX, playerY, tileSize, tileSize, null);
                        } else if (direction.equals("down")) {
                            
                            g2.drawImage(assetManager.getSprite("PLAYER_DOWN"), playerX, playerY, tileSize, tileSize, null);
                        }
                          else if (direction.equals("left")) {
                            // LẬT ẢNH: Vẽ ảnh với chiều rộng âm (-tileSize) và cộng thêm tileSize vào tọa độ X
                            g2.drawImage(assetManager.getSprite("PLAYER"), playerX + tileSize, playerY, -tileSize, tileSize, null);
                        } else {
